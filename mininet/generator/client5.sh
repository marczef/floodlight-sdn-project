...
MIN_BURST=3     # Minimalna przepustowość w Kbps
MAX_BURST=9    # Maksymalna przepustowość w Kbps
...
run_iperf_client() {
    ...
    echo "Uruchamiam klienta $client, który wysyła dane do $server na porcie 100$port przez $duration sekund"
...
generate_random_traffic() {
    ...
    for i in $(seq 1 $ITERATIONS); do
        client=$(shuf -n 1 -e h2)
        server=$(shuf -n 1 -e 10.0.0.4)
